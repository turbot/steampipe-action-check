...
import { GroupResult, ControlResult } from "./annotate";
...
export async function AddPRComments(actionInputs: ActionInput, myExportFile: string) {
  ...
  const group: GroupResult = JSON.parse(content);
  // console.log('--------------->>>>>>>>>', group);
  ParseOnRun(group, actionInputs)
  endGroup()
}

function ParseOnRun(group: GroupResult, actionInputs: ActionInput) {
...
async function AnnotationOnLine(actionInputs: ActionInput, result: ControlResult) {
  try {
    const octokit = new Octokit({
      auth: actionInputs.githubToken
    });
